...
@ApplicationScoped
public class MapConfigBsonPersonProfile {

	final BsonPersonProfileMapStore bsonPersonProfileMapStore;

	@Inject
	public MapConfigBsonPersonProfile(final BsonPersonProfileMapStore bsonPersonProfileMapStore) {
	...
	public final void setConfig(final MapConfig mapConfig) {
		...
		mapStoreConfig.setImplementation(bsonPersonProfileMapStore);
		// Enabled for map
		...
		// mapStoreConfigBsonPerson.setProperty("db.connection.url", "jdbc:postgresql://localhost:5432/mydb") / .setProperty("user", "myuser") / .setProperty("password", "mypass");

		/*
		 * 2) Configure MapConfig and attach MapStoreConfig to MapConfig
		 */
		mapConfig.setName("bsonPersonProfile");
		/*
