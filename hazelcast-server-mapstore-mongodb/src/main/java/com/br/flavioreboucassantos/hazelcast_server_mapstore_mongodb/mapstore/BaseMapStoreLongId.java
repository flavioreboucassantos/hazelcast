...
public class BaseMapStoreLongId<V extends BaseEntityLongId> implements MapStore<Long, V> {
	...
	private Class<V> getSecondTypeArgument() {
		// 1. Get the generic superclass (e.g., MyBase<String, Integer>)
		final Type superclass = getClass().getGenericSuperclass();

		// 2. Extract the actual type arguments
		final ParameterizedType parameterizedType = (ParameterizedType) superclass;
		final Type[] typeArguments = parameterizedType.getActualTypeArguments();

		// 3. Select the second argument (index 0)
		return (Class<V>) typeArguments[0];
	}

	public BaseMapStoreLongId(final MongoDatabase database, final String collectionName) {
		this.collection = database.getCollection(collectionName, getSecondTypeArgument());
